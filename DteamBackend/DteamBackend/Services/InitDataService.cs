    ...
    public class InitDataService : IInitDataService
    {
        ...
        public async Task EnsureGamesSeededAsync(AppDbContext context)
        {
            try
            {
                ...
                // Ensure demo test user exists
                var testUser = await context.Users.FirstOrDefaultAsync(u => u.Email == "user@dteam.io" || u.Username == "testuser");
                if (testUser == null)
                {
                    PasswordHasher.CreatePasswordHash("user123321", out string pHash, out string pSalt);
                    testUser = new Duser
                    {
                        Id = Guid.NewGuid(),
                        Email = "user@dteam.io",
                        ...
                        PasswordHash = pHash,
                        PasswordSalt = pSalt,
                        WalletAddress = "EQB_testuser_wallet_address_777",
                        HardhatAddress = "0x70997970C51812dc3A010C7d01b50e0d17dc79C8",
                        BalanceInNanoTons = 50_000_000_000,
                        TotalEarningsInNanoTons = 0,
                        CreatedAt = DateTime.UtcNow,
                        Status = UserStatus.Online,
                        AvatarUrl = "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=500&auto=format&fit=crop&q=80",
                        Bio = "Тестовий користувач Dteam"
                    };
                    await context.Users.AddAsync(testUser);
                    await context.SaveChangesAsync();
                }

                // Seed user library games if empty
                if (!await context.UserGames.AnyAsync())
                {
                    var mainGames = await context.Games.Where(g => !g.IsDlc).Take(6).ToListAsync();
                    var allUsers = await context.Users.ToListAsync();
                    foreach (var u in allUsers)
                    {
                        for (int i = 0; i < mainGames.Count; i++)
                        {
                            var g = mainGames[i];
                            await context.UserGames.AddAsync(new UserGame
                            {
                                UserId = u.Id,
                                GameId = g.Id,
                                PurchasedAt = DateTime.UtcNow.AddDays(-10 * (i + 1)),
                                PlayTimeMinutes = (i + 1) * 125,
                    ...
                    await context.SaveChangesAsync();
                    _logger?.LogInformation("[InitData] Seeded user library games for test users.");
                }
